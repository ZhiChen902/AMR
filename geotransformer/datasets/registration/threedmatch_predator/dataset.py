...
class ThreeDMatchPairDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.parser_arg = parsed_args
        self.image_path=cfg.train.image_path
        self.geo_test_3DMatch=cfg.train.geo_test_3DMatch
        self.geo_test_3DLoMatch = cfg.train.geo_test_3DLoMatch
    ...
    def __getitem__(self, index):
        ...
        if self.using_geo_prior:
            ...
            if self.parser_arg.use_random_init:
                prior_data = {}
                prior_data['transform'] = np.eye(4)
            else:    
                if self.parser_arg.benchmark == '3DLoMatch':
                    file_name = osp.join(self.geo_test_3DLoMatch, f'{index+1}.npy')
                else:
                    file_name = osp.join(self.geo_test_3DMatch, f'{index+1}.npy')
                prior_data = np.load(file_name, allow_pickle=True).item()
            estimated_transform=prior_data['transform']
