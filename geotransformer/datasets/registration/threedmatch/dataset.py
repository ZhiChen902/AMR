        ...
        if self.using_geo_prior:
            ...
            try:
                ...
                if self.subset == 'train':
                    estimated_transform = np.loadtxt(f"./prior_Geo_train/transformation/{index+1}.txt")    
                elif self.subset == 'val':
                    estimated_transform = np.eye(4)
                    estimated_transform[:3, :3] = rotation
                    estimated_transform[:3,3] = translation
                # elif self.subset == '3DLoMatch':
                #     estimated_transform = np.loadtxt(f"/cvlabdata2/home/zhichen/code/DiffReg-RT-short/prior_Geo/transformation/{index+1}.txt")
                prior_data['estimated_transform'] = estimated_transform
