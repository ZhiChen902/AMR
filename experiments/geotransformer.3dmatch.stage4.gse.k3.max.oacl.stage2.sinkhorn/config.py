...
# data
_C.data = edict()
# _C.data.dataset_root = '/cvlabsrc1/cvlab/dataset_zhichen/data/3DMatch'
import socket
if "yr-cuda11.3" in socket.gethostname():
    _C.data.dataset_root = '/scratch/Yufan/project-diffreg/data2/3DMatch'
elif "Zhifei-PC" in socket.gethostname():
    _C.data.dataset_root = '/home/ubuntu/Documents/point_cloud_registration/data/3DMatch'
else:
    _C.data.dataset_root = '/mnt/F/chenzhi/PEAL/data/3DMatch'
...
import socket
if "yr-cuda11.3" in socket.gethostname() or "Zhifei-PC" in socket.gethostname():
    _C.train.geo_train_prior= './geo_prior_training'
    _C.train.geo_prior= '/mnt/F/chenzhi/PEAL/geo_prior/geo_prior_testval'
else:
    _C.train.geo_train_prior= '/mnt/F/chenzhi/PEAL/geo_prior/geo_prior_training'
    _C.train.geo_prior= '/mnt/F/chenzhi/PEAL/geo_prior/geo_prior_testval'
...
def make_root_dir(parsed_args):
    _C.output_dir = osp.join(_C.root_dir, 'output', _C.exp_name)
    _C.snapshot_dir = osp.join(_C.output_dir, 'snapshots-' + str(parsed_args.train_preset_T))
    _C.log_dir = osp.join(_C.output_dir, 'logs')
    _C.event_dir = osp.join(_C.output_dir, 'events')
    _C.feature_dir = osp.join(_C.output_dir, 'features')
    _C.registration_dir = osp.join(_C.output_dir, 'registration')
